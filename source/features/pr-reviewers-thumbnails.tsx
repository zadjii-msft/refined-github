...
function addToPrList(pullNumber: string, reviewStates: UserStateMap): void
{
	const tableRow = select(`#issue_${pullNumber}`)!;
...
async function getPRState(ownerName: any, repoName: any, pullNumber: string): Promise<void> {
	// Get all the reviews on this pull request
	const result = await api.v3(`repos/${ownerName}/${repoName}/pulls/${pullNumber}/reviews`);
	...
	let userState: UserStateMap = {};
	for (let i = 0; i < result.length; i++) {
		const review = result[i];
		if (review.state) {
			if (userState[review.user.id]) {
				let lastReview = userState[review.user.id];
	...
	addToPrList(pullNumber, userState);
}
...
async function init(): Promise<void> {
	...
	for (const link of links) {
		...
		const pullNumber = pathnameParts[4];
		if (pullNumber) {
			getPRState(ownerName, repoName, pullNumber);
		}
	}
}
